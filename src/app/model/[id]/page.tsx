        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 py-8" role="main">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <h1 className="text-3xl font-bold tracking-tight">{entry.modelName}</h1>
              <span className="inline-flex items-center gap-0.5 px-2 py-0.5 rounded bg-muted text-[10px] font-semibold text-muted-foreground border border-border uppercase">
                {entry.category}
              </span>
              <span className={cn(
              ...
              <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium border', qc.className)}>
                {qc.label}
              </span>
            </div>
            <p className="text-sm text-muted-foreground leading-relaxed mt-1">{entry.description}</p>
            <p className="text-xs text-muted-foreground mt-2">
              By <Link href={`/?provider=${encodeURIComponent(entry.provider)}`} className="hover:text-amber-500 transition-colors font-medium">{entry.provider}</Link> &bull; Version: {entry.version} &bull; Last verified: {entry.lastVerified}
            </p>
          </div>
                  ...
                  <div key={i} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted/30 transition-colors">
                    ...
                    {source.url && (
                      <a
                        ...
                        className="text-amber-500 hover:text-amber-400 text-xs font-semibold shrink-0"
                      >
                        View ↗
                      </a>
                    )}
