...
- **Architecture**: SDXL
- **Native Resolution**: 1024×1024[reference:68][reference:69]
- **Prompt Understanding**: Sensitive to prompt nuances[reference:70]

### Prompt Structure
A well-structured SDXL prompt should follow a structured approach with four key sections[reference:71]:

1. **Subject**: Who or what is in the image[reference:72]
2. **Details**: Appearance, clothing, expression, pose[reference:73]
3. **Environment**: Background, setting, lighting, atmosphere[reference:74]
4. **Style**: Artistic style, medium, quality modifiers[reference:75]

### Quality Tags (Community Standard)
- Positive: \`masterpiece, best quality, very aesthetic, highly detailed\`
...
### Best Practices
- Be specific and detailed — the more specific, the better the chances of getting the desired image[reference:76]
- Use keyword emphasis: \`(word:1.3)\` or \`(word:0.7)\`
- Use photographic language for realism: camera settings, lighting, depth of field[reference:77]
- Different samplers work best — experiment with settings
...
6. Use photographic language for realism

Generate the best SDXL prompt.` + SHORT_VERSION_ENDING,
    sources: [
      { title: 'Ultimate Guide to Creating Realistic SDXL Prompts', url: 'https://civitai.com/articles/11432/ultimate-guide-to-creating-realistic-sdxl-prompts', type: 'guide' },
    ...
    tips: [
      ...
      'Prompt order matters — place most important elements first',
...
- **CFG Scale**: Typically between 5 and 9 — default is 7[reference:114]
- **Prompt Style**: Keyword-based works better than natural language for SD 1.5[reference:115]

### Prompt Structure
For SD 1.5, use keyword-style prompts with comma-separated tags[reference:116]:
...
- **5-9**: Standard range — 7 is default[reference:117]
- **1-2**: For LCM/Lightning variants

### Best Practices
- Be specific and explicit in what needs to be in the picture[reference:118]
- Use 20-40 steps for best balance between quality and speed[reference:119]
...
Generate BOTH positive and negative prompts.` + SHORT_VERSION_ENDING,
    ...
    tips: [
      'SD 1.5 uses keyword-based prompting — short, comma-separated tags work best[reference:120]',
      'Still the most fine-tuned model with the largest LoRA ecosystem',
      'Lower hardware requirements than SDXL',
      'Use 20-40 steps for best balance[reference:121]',
    ],
    lastVerified: '2026-06',
